...
def trainModel(train_op, int_text, train_graph, initial_state, input_text, targets):
    # 获得训练用的所有batch
    batches = get_batches(int_text, batch_size, seq_length)
    # 打开session开始训练，将上面创建的graph对象传递给session
    with tf.Session(graph=train_graph) as sess:
...
def createNumDict():
    ...
    helper.preprocess_and_save_data(lines_of_text, helper.token_lookup, helper.create_lookup_tables)
...
if __name__ == '__main__':
    #int_text, vocab_to_int, int_to_vocab, token_dict = helper.load_preprocess()
    #createModel(int_to_vocab)
    #trainModel(train_op, int_text, train_graph, initial_state, input_text, targets)
    seq_length, load_dir = helper.load_params()
    writeSongs(load_dir, seq_length)
